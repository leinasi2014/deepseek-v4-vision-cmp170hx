...
def build_c128a_topk_metadata(
    ...
    Writes into active-width views of capacity-sized pre-allocated buffers.

    The views deliberately preserve the buffers' capacity row stride. CUDA graphs
    retain tensor strides captured at graph construction, so repacking rows to
    ``max_compressed_tokens`` would make replay read the wrong rows when the
    runtime active width changes.
    """
