...
__global__ void __launch_bounds__(kThreadsPerBlock, 2)
  ...
  for (uint32_t iter = 0; iter < total_iters; iter++) {
    ...
    // would reinterpret as ~4e9 and force the cooperative radix path. Values
    // beyond the physical row stride would also read into the next row.
    // Bounding by max_seq_len keeps this per-row decision consistent with the
    // non-leader CTA early exit above and prevents a leader from waiting on
    // peer CTAs that have already returned.
